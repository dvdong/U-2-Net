...
# ==========================dataset load==========================
# 对图像/标签进行缩放处理
# 将图像和标签都缩放为 (output_size, output_size)
...
class RescaleT(object):
    ...
    def __call__(self, sample):
		...
		# mode='constant' 表示在边缘补 0；
        # order=0 表示使用最近邻插值法；preserve_range=True 表示保持原始范围
...
# 自定义 PyTorch Dataset 类，用于加载图像和标签
# 该类继承自 torch.utils.data.Dataset，并实现了 __init__、__len__ 和 __getitem__ 方法
# __init__ 方法用于初始化数据集，__len__ 方法返回数据集的大小，__getitem__ 方法根据索引获取图像和标签
